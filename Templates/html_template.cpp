 ...
 *
 * Small changes to work better with QT
 * Fredre Hattingh
 * Contact: hattinghfg@tut.ac.za
...
html_template::html_template(const std::string & arg_file_name) 
{
 ...
 Set_Template_File(arg_file_name);
     #if defined(DEBUG) && defined(QT_APP)
     qDebug()<<"New Template defined";
    #endif
}
//----------------------------------------------------------------------------
...
void html_template::Set_Template_File(const std::string arg_file_name)
{
 ...
 #if defined(DEBUG) && defined(QT_APP)
   qDebug() << "Template " << QString::fromStdString(str_tmpl_file_name) << " loaded" ;
 #endif
}
//----------------------------------------------------------------------------
...
//QT only
    ...
    #if defined(DEBUG) && defined(QT_APP)
        qDebug()<<"This is a QT APP";
    #endif

 QFile infile(QString::fromStdString(str_tmpl_file_name)); //Create QFile from resource

 if (!infile.open(QIODevice::ReadOnly))
  {
     qDebug()<<"Could not open file from resource";
     runtime_ex ex("Could not open template file (QT)");
     throw_exception(ex);
  }
...
//Standard C++
 ...
 in_stream.open( str_tmpl_file_name.c_str(), ios::binary );
   ...
   {
        #ifdef DEBUG
        cout << "Template file could not be opened [Process]" ;
        #endif
 ...
 QTextStream in(&infile);
   ...
   str_tmpl_txt = allLines.toStdString();
 ...
 oss << in_stream.rdbuf();  //Dump all into into oss
